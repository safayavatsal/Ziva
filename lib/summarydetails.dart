...
class SummaryDetails extends StatefulWidget {
  ...
  SummaryDetails(this.id, this.cname, this.pname, this.sname, this.oprice,
...
class _SummaryDetailsState extends State<SummaryDetails> {
  ...
  void accessdata() async {
    ...
    try {
      reference
        ..child("Users").child(user.uid).once().then((DataSnapshot snap) {
          setState(() {
            var data = snap.value;
            email = data['email'];
            contact = data['phone'];
 ...
 void quatation() {
   Navigator.of(context).pop();
    ...
    transport
     ...
     var adminoptions = new GmailSmtpOptions()
      ..username = 'ktrajkot@gmail.com'
      ..password = 'Mohammad@7752';
    var admintransport = new SmtpTransport(adminoptions);

    // Create the envelope to send.
    var adminenvelope = new Envelope()
      ..from = 'ktrajkot@gmail.com'
      ..fromName = 'Ziva'
      ..recipients = ['ktrajkot@gmail.com']
      ..subject = 'Your Order'
      ..text = 
          '\nProduct: ' +
          productname +
    ...
    admintransport
        .send(adminenvelope)
        .then((_) => print('Admin email sent!'))
        .catchError((e) => print('Error: $e'));   
        Fluttertoast.showToast(
            msg: "Your Quatation has been sent Successfully",
...
Future<String> Addcategory(BuildContext context) {
    return showDialog(
        context: context,
        builder: (context) {
          return Form(
            ...
            autovalidate: true,
            child: AlertDialog(
              title: Text("Add Quatation Description"),
              content:  TextFormField(
                        validator: (value) =>
                        value.isEmpty ? 'Please Enter the Quatation Details' : null,
                        onSaved: (value) => uname = value,
                        decoration: InputDecoration(
                            hintText: "Quatation Details",
                            labelText: "Quatation Details",
                        ...
                        Row(
                          children: <Widget>[
                            Padding(
                              padding: const EdgeInsets.only(top: 8.0),
                              child: Text("Rs."
                                  '${price = int.parse(widget.nprice) * text}'),
                            ),
                            Padding(
                              padding:
        ...
        bottomSheet: Container(
          ...
          child: Center(
            child: Column(
              ...
              children: <Widget>[
                Text(
                  "Subtotal : " '${price = int.parse(widget.nprice) * text}',
                  style: TextStyle(fontSize: 18.0),
                ),
                Text(
                  "Delievery Charges : " '${charges}',
                  style: TextStyle(fontSize: 18.0),
                ),
                Text(
                  "Total : " '${total = price + charges}',
                  style: TextStyle(fontSize: 18.0),
                ),
                Row(
                      ...
                      onPressed: () {
                        setState(() {
                          if(text >= 1){
                            ...
                            Fluttertoast.showToast(
                                msg: "Please add 10 items to Get Quatation",
                                gravity: ToastGravity.BOTTOM,
                                toastLength: Toast.LENGTH_LONG);
                          }
                        });
